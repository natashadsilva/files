...
def display_digits():
    ...
    image = file.read()
    digits101 = widgets.Image(value=image, format="png", width=300, height=400,)
    return digits101
...
class RenderClassificationMetrics:
    ...
    """
        ...
        self.class_widgets = {
            camera: widgets.Output(
                layout={"border": "1px solid red", "width": "30%", "height": "200pt"}
            )
            for camera in self.cameras
        }
        self.class_status_widget = widgets.Label(
            value="Status", layout={"border": "1px solid green", "width": "60%"}
        )
        ...
        for camera in self.cameras:
            self.graphic[camera] = {
                ...
                "output": widgets.Output(
                    layout={"border": "1px solid green", "height": "200pt"}
                ),
            }
        vbox_elements = [
            [self.graphic[camera]["images"], self.graphic[camera]["output"]]
            for camera in self.cameras
        ]
        vboxes = [
            widgets.VBox(ele, layout={"border": "1px solid blue"})
            for ele in vbox_elements
        ]
                    ...
                    for camera in self.cameras:
                        ...
                        with self.graphic[camera]["output"]:
                            if camera not in metrics["camera_metrics"]:
                                self.class_status_widget.value = "info: No data for camera '{}' on this pass.".format(
                                    camera
                                )
                                ...
                                self.graphic[camera][
                                    "images"
                                ].description = "img/sec {0:3d}".format(
                                    int(images_per_sec)
                                )
                                self.graphic[camera]["images"].bar_style = (
                                    "warning" if images_per_sec > 100 else "info"
                                )
                                # generate interdigit proportions: (certain/grand_total, uncertain/grand_total)
                                grand_total = sum(certain) + sum(uncertain)
                                certain_percent = [
                                    (ele / grand_total) * 100 for ele in certain
                                ]
                                uncertain_percent = [
                                    (ele / grand_total) * 100 for ele in uncertain
                                ]

                                IPython.display.clear_output(wait=True)
                                df = pd.DataFrame(
                                    {
                                        "Digit % (uncertain)": uncertain_percent,
                                        "Digit % (certain)": certain_percent,
                                    },
                                    index=range(len(certain)),
                                )
                                ax = df.plot.bar(rot=0, title=camera)
                                ax.set_ylim(0, 15)
                                time.sleep(0.05)
                        ...
                        self.class_status_widget.value = "{} of {} ts: {}".format(
                            cnt, len(metrics), metrics["timestamp"]
                        )
                        time.sleep(0.1)
            except KeyboardInterrupt:
...
class RenderUncertainImages:
    ...
    def __init__(self, output_uncertain=None, queue_depth=20):
        ...
        self.digits_vbox = widgets.VBox(self.digits, layout={"width": "30%"})
        self.orig_camera_vbox = widgets.VBox([self.camera, self.prep], layout={"width": "100%"})
        self.rework_hbox = widgets.HBox([self.orig_camera_vbox, self.digits_vbox], layout={"width": "400pt"})
        self.button_hbox = widgets.HBox(
            [self.previous_button, self.stop_button, self.next_button]
        )
        self.rework_button_vbox = widgets.VBox([self.rework_hbox, self.button_hbox])
        self.images_hbox = widgets.HBox(
            [self.rework_button_vbox],
    ...
    def render_view(self, tup, status_text):
      ...
      """
        try:
            ...
            # --- 8/3
            #             oimg = widgets.Image(
            #                 value=io.BytesIO(base64.b64decode(ascImg)).getvalue(),
            #                 width=300,
            #                 height=400,
            #             )
            ...
            for idx, x in enumerate(tup["predictions"]):
                ...
                self.digits[idx].layout = (
                    {"border": "2px solid green", "height": "7%"}
                    if tup["result_class"] == idx
                    else {"height": "6%"}
                )
    ...
    def on_button_clicked(self, button):
        ...
        if (
            ...
            and self.image_index < len(self.view_tuples) - 1
        ):
            self.image_index += 1
        ...
        self.render_view(
            json.loads(self.view_tuples[self.image_index]),
            "{} of {}".format(self.image_index + 1, len(self.view_tuples)),
        )
    ...
    def start_thread(self):
        ...
        self.thread = threading.Thread(
            target=self.render_thread, args=(self.fetch_data_name)
        )
        self.thread.start()
...
class CorrectionDashboard:
    ...
    def on_radio_clicked(self, radio):
        ...
        if (
            radio.type == "change"
            and radio.name == "value"
            and not self.init_phase
            and not (self.correct_radio.disabled)
        ):
            self.status.value = "update element {}  old:{} new:{}".format(
                self.image_index, radio.old, radio.new
            )
    ...
    def on_button_clicked(self, button):
        ...
        if button.description == "Training Upload":
            image_new_result = [
                key
                for key in self.corrected_images.keys()
                if str(json.loads(self.view_tuples[key])["result_class"])
                != self.corrected_images[key]
            ]
            self.status.value = "Augment training for {} images".format(
                len(image_new_result)
            )
            ...
            for idx in image_new_result:
                ...
                cnt += 1
                time.sleep(2)
                self.status.value = "image:{} original result_class:{}  updated result:{}".format(
                    idx,
                    json.loads(self.view_tuples[idx])["result_class"],
                    self.corrected_images[idx],
                )
        ...
        self.display_view(
            json.loads(self.view_tuples[self.image_index]),
            "{} of {}".format(len(self.view_tuples), self.image_index),
        )
    ...
    def __init__(self):
        ...
        # --- 8/3
        # camera_vbox = widgets.VBox(
        #     [self.camera, self.orig],
        #     layout={"border": "1px solid blue", "width": "33%"},
        # )
        predict_vbox = widgets.VBox(
            [self.camera, self.result, self.prep],
            layout={"border": "1px solid green", "width": "50%"},
        )
        radio_vbox = widgets.VBox(
            [self.correct_radio], layout={"border": "1px solid blue", "width": "50%"}
        )

        images_hbox = widgets.HBox([predict_vbox, radio_vbox])

        rework_hbox = widgets.HBox(
            [
                self.previous_rework,
                self.next_rework,
                self.commit_rework,
                self.progress_rework,
            ]
        )
        controls_vbox = widgets.VBox(
            [images_hbox, rework_hbox], layout={"border": "2px solid black"}
        )
    ...
    def display_view(self, tup, status_text):
        try:
            ...
            with self.prep:
                ...
                plt.show()
            radio_buttons = [
                "%d:%9.5f" % (idx, x) for idx, x in enumerate(tup["predictions"])
    ...
    def render_review(self, view_tuples):
        ...
        self.image_index = 0
        self.display_view(
            json.loads(view_tuples[self.image_index]),
            "Reviewing {} images".format(len(view_tuples)),
        )
